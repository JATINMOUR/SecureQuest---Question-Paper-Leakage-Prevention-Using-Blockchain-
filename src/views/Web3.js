...
App = {
  ...
  downloadQuestions: function () {
    ...
    var totalQues = parseInt(document.querySelector(".questionCount").value, 10);
    var weightage = parseInt(document.querySelector(".weightage").value, 10);
    var count1 = parseFloat(document.querySelector(".perCount").value);
    // console.log("prec count");
    // console.log(count1);

    if (!date || subject == "Choose subject" || !totalQues || isNaN(weightage) || isNaN(count1)) {
      swal(
        "Missing input",
        "Please select date, subject, number of questions, chapter weightage, and accepted-question percentage.",
        "warning"
      );
      return;
    }
    ...
    if (count1 < 0 || count1 > 100) {
      swal(
        "Invalid percentage",
        "Percentage of your accepted questions should be between 0 and 100.",
        "warning"
      );
      return;
    }
    ...
    if (excludedTopics.length == 0) {
      excludedTopics = new Array(chapterArray.length).fill(-1);
    }
    ...
    App.contracts.CrowdSource.deployed()
      ...
      .then(function (problemCount) {
        var totalProblems = problemCount.toNumber ? problemCount.toNumber() : parseInt(problemCount, 10);
        if (totalProblems == 0) {
          problemCard.innerHTML = `<div class="card">
            <div class="card-body">
              No questions are available yet. Please add and approve questions before generating a paper.
            </div>
          </div>`;
          swal(
            "No questions available",
            "Please add and approve questions before generating a paper.",
            "info"
          );
          return;
        }
        ...
        for (var i = 1; i <= totalProblems; i++) {
          requests.push(crowdsourceInstance.problems(i).then(function (p) {
            return p;
          }));
        }

        return Promise.all(requests).then(function (problems) {
          let requestedOwnCount = Math.round((count1 * totalQues) / 100);
          let selected = [];
          let selectedKeys = new Set();

          function canUseQuestion(p) {
            if (!(p[6] == true && p[7] == true && p[0] == subject)) {
              return false;
            }

            var chapterIndex = chapterArray.indexOf(p[1]);
            if (chapterIndex == -1 || excludedTopics[chapterIndex] != -1) {
              return false;
            }

            return map.get(p[1]) < weightage;
          }

          function addQuestionToPaper(p, key) {
            if (!canUseQuestion(p) || selected.length >= totalQues || selectedKeys.has(key)) {
              return false;
            }

            map.set(p[1], map.get(p[1]) + 1);
            selected.push({ key: key, problem: p });
            selectedKeys.add(key);
            return true;
          }

          problems.forEach(function (p, idx) {
            if (selected.length >= requestedOwnCount) {
              return;
            }

            if (p[9] && p[9].toLowerCase() == App.account.toLowerCase()) {
              addQuestionToPaper(p, idx);
            }
          });
          ...
          if (selected.length == 0) {
            problemCard.innerHTML = `<div class="card">
              <div class="card-body">
                No approved questions matched the selected subject and chapters.
              </div>
            </div>`;
            swal(
              "No questions found",
          ...
          let quesData = "";
          questionStored = selected.map(function (item) {
            var p = item.problem;
            return {
              subject: p[0],
              topic: p[1],
              question: p[2],
              options: App.getOptions(p[3]),
              imgUrl: p[4],
              ans: p[5].toNumber(),
              approve: p[6],
              isApproved: p[7],
          ...
          selected.forEach(function (item, index) {
            ...
            quesData += `<br />
                    <div class="card fullCardQP${item.key}">
                      <div style="background-color: rgb(221, 221, 221)" class="card-header">
                        <p class="ques" contenteditable="true">
                          Ques${questionNumber}. ${p[2]}
                        </p>
                      </div>
                      <br>
                      <img width="35%" style="margin-left: 5%;" src='${p[4]}' alt="">
                      <div class="card-body">
                        <table>
                          <tr>
                        ...
                        <i class="fa fa-plus" id="addBlock${item.key}" onclick="addRemove(${item.key},${questionNumber})" aria-hidden="true"></i>
                        <i class="fa fa-minus" id="removeBlock${item.key}" onclick="addRemove(${item.key},${questionNumber})" aria-hidden="true"></i>
                      </div>
                    </div>`;
          });
