...
function setWeightage() {
  var total_question = parseInt(document.querySelector(".questionCount").value, 10);
  var subject = document.querySelector(".subject").value;
  if (!total_question || subject == "Choose subject") {
    return;
  }
  ...
  if (excludedTopics.length == 0) {
    ...
    excludedTopics = new Array(topicCounts[subject] || 0).fill(-1);
  }
  ...
  var max_weightage = Math.max(1, Math.ceil(total_question / chapterCount));
